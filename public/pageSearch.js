...
function displayDives(dives) {
    ...
    for (let dive of dives) {
        ...
        diveLiDate.className = "dive-li"
        diveLiDate.innerText = new Date(dive.date.slice(0,10)).toDateString().slice(0,16);
        diveUl.appendChild(diveLiDate);
        
        buttonForm(diveBody, dive, "/dives/details", "See Dive Details")
        buttonForm(diveBody, dive,  "/dives/edit", "Update Dive")
        buttonForm(diveBody, dive, "/dives/deleteDive?_method=DELETE", "Delete Dive")
    }
    function buttonForm (diveBody, dive, action, inText) {
